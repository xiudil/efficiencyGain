...
#          nuisance functions in AIPW estimated via super learner
one <- function(which.W,P){
  set.seed(P)
  data = data[sample(1:nrow(data),nrow(data),replace = T),]
  if (which.W =="bmi" | "bmi" %in% which.W ){
...
allP <-  colnames(data)[c(3,6,18,19,22,23,24,25,27)]
...
###   on a dataset simulated from the original data
###   using AIPCW estimator adjusting for covariates in which.W
###   as well as using unadjusted estimator
#          nuisance functions in AIPCW estimated via super learner
