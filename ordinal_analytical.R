...
#####################################
### ordinal: estimating gain + CI ###
#####################################

### additional gam learners in the super learner
### can include other learning algorithms
learners = create.Learner("SL.gam",tune=list(deg.gam = c(5,10,20)))

### w: a data.frame of the covariates
### y: a vector of the outcome
### u: monotone transform on the outcome in difference in mean, u=I for the sims
### pevent_mod, cdf_mod: conditional pdf and cdf of Y|W, estimated through proportional odds model
### link: scale on which RE estimate and CI are reported, see the example below
### cf: if true, nuisance functions are estimated using 10-fold cross fitting
...
### data generating mechanism, setting I
page = c(0.01,0.09,0.12,0.13,0.18,0.22,0.25)
pevent = matrix(c(0,0.01,0.03,0.08,0.11,0.17,0.37,0,0.18,0.32,0.31,0.37,0.47,0.35,
                  rep(0,7)),ncol=3,byrow=FALSE)
pevent[,3] = 1-pevent[,1]-pevent[,2]

### data generating mechanism, setting II
